...
public class GdxAdaptor implements com.badlogic.gdx.ApplicationListener, EngineState {
	...
	private StateSwitcher<ENGINE_STATE> state;
	private DebugTimer timer;

	public GdxAdaptor (UnitsMachineExecutor executor) {
		this.executor = executor;
		state = JUtils.newStateSwitcher(ENGINE_STATE.NEW);
	}
	...
	@Override
	public void render () {
		if (state.currentState() == ENGINE_STATE.OK) {
			this.do_full_cycle();
			return;
		}
		if (state.currentState() == ENGINE_STATE.RESTORING) {
			this.do_resume();
			this.setState(ENGINE_STATE.OK);
			this.do_full_cycle();
			return;
		}
		if (state.currentState() == ENGINE_STATE.NEW) {
			this.deploy();
			this.setState(ENGINE_STATE.OK);
			this.do_full_cycle();
			return;
		}
		if (state.currentState() == ENGINE_STATE.BROKEN) {
	...
	private void deploy () {
		state.expectState(ENGINE_STATE.NEW);
		Screen.installComponent(this.viewport_state);
		fokker_render_machine = new FokkerRenderMachine();
		RenderMachine.installComponent(fokker_render_machine);
		// ------------------------------
		this.executor.doDeploy();
		Gdx.input.setInputProcessor(input_adaptor);
	...
	private void setState (ENGINE_STATE next_state) {
	...
	public boolean isBroken () {
		return state.currentState() == ENGINE_STATE.BROKEN;
	}
}
