...
public class EllipseRenderer {
	...
	private void drawGdxSegment (final ReadOnlyFloat2 center, final ReadOnlyFloat2 A, final ReadOnlyFloat2 B, final boolean filled) {
		this.x1 = SpritesRenderer.round(A.getX());
		this.y1 = SpritesRenderer.round(A.getY());
		this.x2 = SpritesRenderer.round(B.getX());
		this.y2 = SpritesRenderer.round(B.getY());
		if (!filled) {
			GdxRender.shapeRenderLine(this.x1, this.y1, this.x2, this.y2);
		} else {
			this.x3 = SpritesRenderer.round(center.getX());
			this.y3 = SpritesRenderer.round(center.getY());
