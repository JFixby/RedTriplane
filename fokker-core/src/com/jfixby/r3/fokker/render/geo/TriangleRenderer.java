...
import com.jfixby.r3.fokker.render.raster.SpritesRenderer;
...
public class TriangleRenderer {
	...
	private void drawTriangle (final Float2 a, final Float2 b, final Float2 c) {
		final float ax = SpritesRenderer.round(a.getX());
		final float ay = SpritesRenderer.round(a.getY());

		final float bx = SpritesRenderer.round(b.getX());
		final float by = SpritesRenderer.round(b.getY());

		final float cx = SpritesRenderer.round(c.getX());
		final float cy = SpritesRenderer.round(c.getY());
